...
In diesem Kapitel wollen wir nichts anderes tun als Farben zu erzeugen und sie zu mischen um neue Farben zu erzeugen.
Um das tun zu können schauen wir uns zuerst einmal an, wie das ganze funktioniert.

## Die Physik der Farben

Vereinfacht gesagt gehen wir in diesem Modell davon aus, dass es die drei Grundfarben Rot, Grün und Blau gibt, aus denen sich alle anderen Farben erzeugen lassen. Um euch davon zu überzeugen, dass das nicht ganz falsch zu sein scheint müsst ihr einfach nur auf euer Computerdisplay schauen. Jeder Pixel auf eurem Display besteht aus drei Farbpunken die unterschiedlich hell leuchten und damit einen unglaublich großen Farbbereich erzeugen können. Dabei ist es erstmal egal ob `OLED` oder `TFT`. Die theoretischen Grundlagen für das Mischen von Licht bleiben die gleichen.
...
Jeder Pixel hat als eine `uint8_t` Variable für Rot, Grün und Blau. Das `uint8_t`  steht für den Typen der Variable, `red`, `green`und `blue` sind die Namen der Variablen. 
Wenn ihr nicht wisst was Variablen sind. Hier steht genauer was das bedeutet [Variablen](Grundlagen.md#variablen)
...
Es wird Zeit dass wir das alles mal selbst ausprobieren. Öffnet das Beispiel `Datei|Beispiele|Jugend Hackt Animation-lib|Farben` und schaut euch den Quelltext an.
...
Das alles sieht erstmal nach viel aus, aus diesem Grund schauen wir uns alles Schritt für Scritt an, dann werden wir sehen, dass das alles garnicht so kompliziert ist wie es vielleicht im ersten Moment aussieht.
...
Danach stelllen wir die Farben so ein, dass jeweils die rote, grüne und blaue led ganz hell ist (`255`) und alle anderen ganz dunkel (`0`).
...
Ihr kennt alle Fußgängerampeln. Wenn man den Knopf an der Ampel drückt schaltet sie für eine Kurze Zeit von Rot aud Grün.
...
Eine Bedingung ist soetwas wie *wenn, dann*. Auf eglisch heißt das *if, then* und für den Arduino eben `if`.
...
Wir definieren unsere beiden Farben `rot` und `grün`. Dann schreiben wir unsere Bedingung *wenn farbe gleich rot, dann* oder `if(farbe == rot) {`.
...
Man kann unsere Bedingung auch mit einem *sonst* Fall erweitern, also *wenn, dann, sonst*. Auf egnlisch also *if, then, else* oder für den Arduino `if(){}else{}`. Wie das mit dem *sonst* Fall genau funktioniert ist [hier](Grundlagen.md#bedingungen-und-alternativen) etwas genauer beschrieben.
...
Ist doch ganz einfach. Wie machen wir das jetzt mit userer Ampel?
...
Was steht jetz genau da? Wenn die `farbe` `rot` ist dann ändern wir die `farbe` auf `gruen`.
Wenn die Farbe nicht `rot` ist, dann ändern wir die `farbe` auf `rot`.
Ist doch garnicht schwer.

### Beispiel

Öffnet das Beispiel `Datei|Beispiele|Jugend Hackt Animation-lib|FussgaengerAmpel` und schaut euch den Quelltext an.
...
Die Methode `loop()` wird vom Arduino immer wieder ausgefürt und alle Werte in der Methode werden danach aufgeräumt.
...
Öffnet das Beispiel `Datei|Beispiele|Jugend Hackt Animation-lib|FarbenMathe` und schaut euch den Quelltext an.
...
Die Parameter für `FastLED` und usere `setup()` Methode haben sich nicht verändert. Aber unsere `loop()` Methode schaut etwas anderst aus. Was hat sich denn da verändert?

``` C++
  uint8_t basis = 127;
```

Zuerst definieren wir eine Variable mit dem Namen `basis` vom Typ `uint8_t`. Das ist der selbe Typ, der in der Struktur `CRGB` für die Farben Rot, Grün und Blau verwendet wird. Wir weißen dieser Variable den Wert `127` zu. Wenn ihr nich ganz versteht was genau da passiert könnt ihr auch [hier](Grundlagen.md#variablen) nocheinmal nachschauen wie genau das funktioniert.

Die nächsten Zeilen definieren verschiedene Farben, deren Rot wert durch Berechungen mit der variable `basis` entstehen.
Wie das mit mathematischen Berechungen mit Variablen funktioniert wird [hier](Grundlagen.md#beispiel-rechnen-mit-variablen) nochmal genau gezeigt. Aber schauen wir uns die Schritte man im Detail an.
...
Das ist was hier passiert. Wenn euch das zu schnell geht könnt ihr auch [hier](Grundlagen.md#die-mathematik) nocheinmal nachlesen wie das mit dem Rechnen auf dem Arduino funktioniert.
...
Wir können auch mehrere Grundfarben von userem `basis` Wert ableiten.
...
Öffnet das Beispiel `Datei|Beispiele|Jugend Hackt Animation-lib|FarbenMischen` und schaut euch den Quelltext an.
...
Aber die `loop()` Methode hat sich wieder verändert.
...
So wit so einfach.
Im nächsten Schritt beginnen wir Farben zu mischen und zwar nennt sich das hier *Additive Farbnischung*. Wir addieren verschiedene Farben mit einander um neue Farben zu erzeugen. Genau nach diesem Prinzip funktioniert auch euer Monitor.
...
Hier ziehen wir von Weiß die Farbe Grün ab. Also bleiben rot und blau übrig, was Magentha ist.
Oder wir ziehen von der Farbe Weiß magenta ab und es bleibt logischerweise grün übrig, daMagenta aus Blau und Rot besteht.
...
Öffnet das Beispiel `Datei|Beispiele|Jugend Hackt Animation-lib|EinPixelAmpel` und schaut euch den Quelltext an.
...
Sicher habt ihr euch auch schon gefragt was die ersten Zeilen in den bisherigen Sketches denn eigentlich so machen. Schauen wir uns das Ganze doch einfach mal etwas genauer an.
...
In der letzten Zeile des Beispiels definieren wir ein Array aus Farben, welches die Länge `NUM_LEDS` hat.
...
Öffnet das Beispiel `Datei|Beispiele|Jugend Hackt Animation-lib|FarbenMehrere` und schaut euch den Quelltext an.
...
Das sieht schon ein wenig anderst aus als vorher oder?.
...
Die Variable wir hier verwendet um die Größe des Arrays einzustellen.
...
Das kennen wir noch aus dem letzen Sketch. Wir definieren ein paar Variablen mit Farbwerten, die wir dann später unseren LEDs zuweisen. Soweit eigentlich alles bekannt.
...
Dafür sollten wir aber überlegen welche Zustände unsere Ampel eingentlich hat.
...
Schauen wir uns ersteinmal an wie wir von Grün zu Gelb und dann zu Rot wechseln können.
...
Wie das in Form eines Sketches aussieht seht ihr im Beispiel `Datei|Beispiele|Jugend Hackt Animation-lib|MehrPixelAmpel`
...
Wie oben bereits besprochen definieren wir die Fatben `rot`, `gelb` und `gruen`.
...
Da wir als erstes schauen ob die erste LED grün leuchtet und das jetzt immer stimmt schalten wir einfach immer wieder die gelbe LED an und es passiert nichtsmehr.
...
Das wäre natürlich nicht hilfreich weil wir bisher immer die rote LED einschalten, wenn die gelbe LED leuchtet, wie ihr in der zweiten Befingung oben sehen könnt.
...
Den gibt es tatsächlich. Wir können Bedingungen in einander verschachteln. Also in eine Bedingung eine weitere Bedingund einbauen.
...
Wenn die erste Bedingung also zutrifft finden wir dann die zweite Bedingung zwischen den Klammern `{` und `}`, die zur ersten Bedingung gehören.
...
Wie das in Form eines Sketches aussieht seht ihr im Beispiel `Datei|Beispiele|Jugend Hackt Animation-lib|VierSchritteAmpel`
...
Mit Eurem Wissen aus der Erklährung weiter oben sollte der Sketch für euch mittlerweile kein Problem mehr darstellen.
...
Wie ihr sehen könnt kann man mit dem Arduino und ein paar LEDs ganz schön komplexe Dinge bauen, wenn man über ein paar wenige Werkzeuge ein bissche bescheid weiss, dann ist das auch alles garnicht so schwer wie man das vielleicht am Anfang denkt.
...
Das ist wirklich viel und im Nächsten kapitel seht ihr dann was man mit diesem ganzen Wissen noch machen kann, nämlich Animationen.
